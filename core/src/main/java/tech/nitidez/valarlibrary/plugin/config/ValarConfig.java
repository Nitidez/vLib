package tech.nitidez.valarlibrary.plugin.config;
...
public class ValarConfig {
  
  private static final Map<String, ValarConfig> cache = new HashMap<>();
  private final ValarPlugin plugin;
  private final File file;
  private YamlConfiguration config;
  
  private ValarConfig(ValarPlugin plugin, String path, String name) {
    this.plugin = plugin;
  ...
  public static ValarConfig getConfig(ValarPlugin plugin, String path, String name) {
    if (!cache.containsKey(path + "/" + name)) {
      cache.put(path + "/" + name, new ValarConfig(plugin, path, name));
    }
